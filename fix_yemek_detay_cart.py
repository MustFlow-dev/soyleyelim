...
yemek_detay_path = r'c:\\Users\\cagat\\Desktop\\Phyton\\soyleyelim\\core\\templates\\core\\yemek_detay.html'
...
with open(yemek_detay_path, 'r', encoding='utf-8') as f:
    content = f.read()

# 1. Update Cart Button
# The button in yemek_detay.html is:
# <a href="{% url 'sepet_detay' %}" class="btn-close-white" style="left: auto; right: 20px;">
if 'data-bs-target="#cartOffcanvas"' not in content:
    # Use strict replacement for this specific tag
    content = content.replace(
        'href="{% url \'sepet_detay\' %}" class="btn-close-white" style="left: auto; right: 20px;"',
        'href="javascript:void(0)" class="btn-close-white" style="left: auto; right: 20px;" data-bs-toggle="offcanvas" data-bs-target="#cartOffcanvas"'
    )
    print("Updated cart button link in yemek_detay.html")

# 2. Append Offcanvas HTML
if 'id="cartOffcanvas"' not in content:
    if '</body>' in content:
        content = content.replace('</body>', offcanvas_html + '\n</body>')
        print("Inserted offcanvas before body end in yemek_detay.html")
    else:
        print("Could not find body tag in yemek_detay.html")
else:
    print("Offcanvas already exists in yemek_detay.html")

with open(yemek_detay_path, 'w', encoding='utf-8') as f:
    f.write(content)
